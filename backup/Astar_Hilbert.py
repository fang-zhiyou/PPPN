...
class AStar:
    ...
    def is_en_point(self, p: V4):  # 可以简化
    ...
    def process_v4(self, p: V4):
        x, y = hilbert_trans.d2xy_up(7, p.v4[0])
        print(f"point {x}, {y}")
    ...
    def run_algorithm(self):
        ...
        while True:
            ...
            # 处理邻居 d0
            d0_v = p.v4[0]
            if d0_v != 0 and dict0[d0_v - 1][4] == 0:  # left
                new_p = V4(dict0[d0_v - 1])
                new_p.base_cost = p.base_cost
                self.process_v4(new_p)
            if d0_v != self.mp.long - 1 and dict0[d0_v + 1][4] == 0:
                new_p = V4(dict0[d0_v + 1])
                new_p.base_cost = p.base_cost
                self.process_v4(new_p)
            ...
            # 处理邻居 d1
            d1_v = p.v4[1]
            if d1_v != 0 and dict1[d1_v - 1][4] == 0:  # left
                new_p = V4(dict1[d1_v - 1])
                new_p.base_cost = p.base_cost
                self.process_v4(new_p)
            if d1_v != self.mp.long - 1 and dict1[d1_v + 1][4] == 0:
                new_p = V4(dict1[d1_v + 1])
                new_p.base_cost = p.base_cost
                self.process_v4(new_p)
            ...
            # 处理邻居 d2
            d2_v = p.v4[2]
            if d2_v != 0 and dict2[d2_v - 1][4] == 0:  # left
                new_p = V4(dict2[d2_v - 1])
                new_p.base_cost = p.base_cost
                self.process_v4(new_p)
            if d2_v != self.mp.long - 1 and dict2[d2_v + 1][4] == 0:
                new_p = V4(dict2[d2_v + 1])
                new_p.base_cost = p.base_cost
                self.process_v4(new_p)
            ...
            # 处理邻居 d3
            d3_v = p.v4[3]
            if d3_v != 0 and dict3[d3_v - 1][4] == 0:  # left
                new_p = V4(dict3[d3_v - 1])
                new_p.base_cost = p.base_cost
                self.process_v4(new_p)
            if d3_v != self.mp.long - 1 and dict3[d3_v + 1][4] == 0:
                new_p = V4(dict3[d3_v + 1])
                new_p.base_cost = p.base_cost
                self.process_v4(new_p)
...
if __name__ == '__main__':
    ...
    # 128 * 128
    graph_data = np.load('../graph_data.npy') # 0 通, 1 不通
    ...
    for i in range(2 ** N):
        for j in range(2 ** N):
            ...
            ii = i
            jj = j
            dict0[d0] = [d0, d1, d2, d3, graph_data[ii, jj]]
            dict1[d1] = [d0, d1, d2, d3, graph_data[ii, jj]]
            dict2[d2] = [d0, d1, d2, d3, graph_data[ii, jj]]
            dict3[d3] = [d0, d1, d2, d3, graph_data[ii, jj]]
            # if graph_data[ii, jj] == 0:
            #     print(i, j)
